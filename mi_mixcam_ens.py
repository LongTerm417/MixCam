# coding=utf-8
"""Implementation of MixCam ensemble attack."""
# MixCam before scale
...
from log_config import HiddenPrints
from func import *
...
tf.flags.DEFINE_integer('percentile', 75, "Name of the model")
...
percentile_list = [i for i in range(1, 100) if (i % 10) == 0]
...
def cam_arguement(x, cam_value):
    eps = 2.0 * FLAGS.max_epsilon / 255.0
    arguement_value = [tf.random.uniform(x.get_shape().as_list(), minval=-eps, maxval=eps, dtype=tf.dtypes.float32)
                       + (1 - FLAGS.portion) * x
                       + FLAGS.portion * percentile_value(cam_value, down_p=dp, step_v=10)
                       for dp in percentile_list]
    return tf.concat(arguement_value, axis=0)
...
def ensemble_cam(x, y):
    ...
    for model_name in models_names:
        logits_v3, end_points_inc_v3 = get_model_results(x=x, model_name=model_name, num_classes=num_classes)
...
def graph(x, y, i, x_max, x_min, grad):
    ...
    x_enhanced = cam_arguement(x, cam_value)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        # logits_v3, end_points_v3 = get_model_results(x=x_input, num_classes=1001)
        # pred = tf.argmax(end_points_v3['Predictions'], 1)
